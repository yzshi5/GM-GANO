...
def BaselineCorrection(time, vel, disp, n=6, f_taper_beg=0.05, f_taper_end=0.10):
    ...
    t_const = np.array([time.min(), time.max()])
    ...
    A = np.vstack([A_disp, A_vel])
    b = np.hstack([disp, vel])
    ...
    #combine projection matrices and signals
    A_const = np.vstack([A_disp_const, A_vel_const])
    b_const = np.hstack([disp[[0,-1]], vel[[0,-1]]])
    
    
    #baseline fit
    p = cp.Variable(n) #polynomial coeffs
    obj   = cp.Minimize(cp.sum_squares(A @ p - b))
    const = [A_const @ p == b_const, ]
