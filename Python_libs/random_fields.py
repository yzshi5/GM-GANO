...
class GaussianRF_idct(object):
    ...
    def __init__(self, dim, Ln, alpha=2.0, tau=3.0, cal1d=False, device=None):
        self.dim = dim
        self.Ln = Ln
        self.device = device
        self.cal1d = cal1d
    ...
    def _sample2d(self):
        ...
        if cal1d == True:
        :return: GRF numpy.narray (Ln, 1)
        """
        # # sample from normal discribution
        if self.cal1d:
            xr = np.random.standard_normal(size=(self.Ln,1))
        else:
    ...
    def sample(self, N, mul=1):
        if self.cal1d:
            z_mat = np.zeros((N,self.Ln, 1), dtype=np.float32)
        else:
